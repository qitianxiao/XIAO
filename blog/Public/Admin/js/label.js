...
layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex=1, pageSize=10) {
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>标签id</th><th>标签名称</th><th colspan="2">选项</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                html += "<tr>";
                html += "<td>" + item.label_id + "</td>";
                html += "<td>" + item.lable_name + "</td>";
                ...
                html += '<td><a class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.datalist.editData(\'' + item.id + '\')" href="http://www.blog.com/index.php/Admin/Label/update?id=' + item.label_id + '"><i class="layui-icon">&#xe642;</i></a></td>';
                html += '<td><a class="layui-btn layui-btn-small layui-btn-danger" onclick="layui.datalist.editData(\'' + item.id + '\')" href="http://www.blog.com/index.php/Admin/Label/delete?id=' + item.label_id + '"><i class="layui-icon">&#xe640;</i></a></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
