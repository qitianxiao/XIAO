...
layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex=1, pageSize=10) {
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>博文id</th><th>博文标题</th><th>博文内容</th><th>博文作者</th><th>博文分类</th><th>博文添加时间</th><th colspan="2">选项</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                html += "<tr>";
                html += "<td>" + item.article_id + "</td>";
                html += "<td>" + item.article_title + "</td>";
                html += "<td>" + item.article_content + "</td>";
                html += "<td>" + item.article_author + "</td>";
                html += "<td>" + item.category_name + "</td>";
                html += "<td>" + item.article_create_time + "</td>";
                ...
                html += '<td><a class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.datalist.editData(\'' + item.id + '\')" href="http://www.blog.com/index.php/Admin/Article/update?id=' + item.article_id + '"><i class="layui-icon">&#xe642;</i></a></td>';
                html += '<td><a class="layui-btn layui-btn-small layui-btn-danger" onclick="layui.datalist.editData(\'' + item.id + '\')" href="http://www.blog.com/index.php/Admin/Article/delete?id=' + item.article_id + '"><i class="layui-icon">&#xe640;</i></a></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
